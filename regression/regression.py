...
def plot_predictions(y_test, y_pred, output_path="salary_plot_full.png"):
    plt.figure(figsize=(8, 6))
    plt.scatter(y_test, y_pred, alpha=0.5)
    plt.plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], '--r')
    plt.xlabel("Actual Salary")
    plt.ylabel("Predicted Salary")
    plt.title("Predicted vs Actual Salary (All Sources)")
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    print(" Salary Prediction (All Sources)")
    ...
    print(" Real vs Predicted (первые 5):")
